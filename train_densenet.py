...
#  models.resnet3d import ResNet3d as ResNet
from models.densenet3d import DenseNet_Cam as DenseNet
...
def parse_args():
    ...
    parser.add_argument('--log_dir', default='logs/DenseNet')
...
def load_ckpt(args, model, optim=None, lr_scheduler=None):
    ...
    if args.local_rank == 0 or args.local_rank == 1 or args.local_rank == 2:
...
def train_epoch(args, epoch, model, loss_fn, optim, dataloader, lr_scheduler=None):
    ...
    for step, sample in enumerate(dataloader):
        ...
        if args.local_rank == 0 or args.local_rank == 1 or args.local_rank == 2:
            ...
            # print(f'[TRAIN] Epoch {epoch}@{step / len(dataloader) * 100:.1f}% | loss:{loss_m.value:.4f}, MAE:{mae_m.value:.4f}, read_time:{read_m.value:.3f}s, calc_time:{calc_m.value:.3f}s')
        timer.tiktok()
...
def train(args):
    ...
    train_loader = DataLoader(train_set, args.bs, num_workers=4, sampler=train_sampler, drop_last=True)
    dev_loader = DataLoader(dev_set, args.bs, num_workers=4, shuffle=False)
