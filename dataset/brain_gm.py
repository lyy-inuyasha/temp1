...
class BrainSet(Dataset):
    def __init__(self, root_dir='/home/hefb/data/T1/data', mode='train', transform=None) -> None:
        super().__init__()
        assert mode in ['train', 'valid']

        info_pd = pd.read_csv(f'{root_dir}/{mode}_info.csv')

        self.ids = info_pd['subject_id'].to_list()
        self.labels = info_pd['age'].to_list()
        self.data_dir = f'{root_dir}/train' if mode != 'valid' else f'{root_dir}/valid'

        self.transform = transform

    def __getitem__(self, index):
        data_path = osp.join(self.data_dir, str(self.ids[index]), 'smwp1T1_resample.nii.gz')
