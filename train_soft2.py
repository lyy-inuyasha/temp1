...
def parse_args():
    ...
    parser.add_argument('--log_dir', default='logs/SFCN_Modified')
...
def train_epoch(args, epoch, model, loss_fn, optim, dataloader, lr_scheduler=None):
    ...
    for step, sample in enumerate(dataloader):
        data, label = sample['data'].cuda(), sample['gt'].cuda()
...
def train(args):
    ...
    train_set = DisBrainSet(mode='val', transform=train_transform)
