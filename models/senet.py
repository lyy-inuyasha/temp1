...
class SEBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        e = self.excitation(s).view(bs, c, 1, 1, 1)
        return x * e.expand_as(x)


class SENet(nn.Module):
    def __init__(self, input_channel=4, n_classes=3):
        super().__init__()
        self.mode = 'single'
        self.b1 = nn.Sequential(
            nn.Conv3d(input_channel, 64, kernel_size=7, stride=2, padding=3),
...
if __name__ == '__main__':
    model = SENet(input_channel=4,n_classes=3)
